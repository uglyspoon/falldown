<template>
  <div class="china-map" id="china-map"  ref="chinaMap">
    <!--中国地图-->

    <div class="margin-top">
      <div class="header">
        <div class="title">十八大以来落马中管干部地域分布</div>
        <div class="rank"></div>
      </div>
      <div class="map-area">
        地方：
  ...
  export default {
    ...
    methods: {
      prepareData () {
        ...
        let rate = width / 750
        this.chartData.reduceHeight = $('.china-map .margin-top').height()
        ...
        this.chartData.otherTop = width / 375 * 330
        this.chartData.colNumber = 25
        this.chartData.mapGroupTop = width * 0.15
        this.chartData.mapTop = -width * 0.11
        ...
        this.chartData.marginTop = $(this.$el).find('.margin-top').height() * 2
      ...
      renderContent () {
        ...
        testMap.attr('transform', () => {
          return `translate(0,${chartData.reduceHeight}) scale(${chartData.rate}, ${chartData.rate})`
        })
        .style('opacity', 0)
        .transition()
        .duration(1000)
        ...
        this.mapSvgPolygons = mapSvgGroups.selectAll('polygon.map-polygon')
          ...
          .attr('transform', `translate(0, ${-chartData.reduceHeight})`)
          .attr('points', (d) => {
            let points = getRegularHexagon(d.grid, this.chartData.width * 0.03)
          ...
          .each(function (d) {
            ...
            d3.select(this)
              ...
              .attr('points', function (d) {
                let points = getRegularHexagon(d.grid, chartData.width * 0.03)
                return _.map(points, (e) => {
                  return e.join(',')
                }).join(' ')
              })
...
<style  lang="scss">
  ...
  .other-label{
	...
	.st11{display:inline;fill:#999999;stroke:#FFFFFF;stroke-miterlimit:10; stroke:#5B1111;}
