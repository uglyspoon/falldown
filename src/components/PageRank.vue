<template>
  <div class="page-rank" id="page-rank" ref="pageRank">
    <!--<div class="title">据财新统计，十八大来落马的中管干部至少182名。</div>-->
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      prepareData () {
        ...
        this.chartData.r = this.chartData.width * 0.027
        this.chartData.xR = Math.cos(Math.PI / 6) * this.chartData.r
        this.chartData.yR = this.chartData.r
        this.chartData.startLeft = this.chartData.width * 0.21
        ...
        arr = _.map(arr, (d, i) => {
          ...
          let t = 0
          _.each(tmp, (e) => {
            t += Math.ceil(e.value.length / 10) * (this.chartData.rowGapY + 2 * this.chartData.yR)
          })
...
<style lang="scss">
  ...
  .page-rank .rank-active {
    transform: scale3d(0, 0,1);
  }
  .page-rank .rank-active:nth-child(1){
    animation: bobble 0.8s linear 0.1s infinite;;
    will-change: opacity;
  }

  .page-rank .rank-active:nth-child(2){
    animation: bobble 0.8s linear 0.4s infinite;;
    will-change: opacity;
  }

  .page-rank .rank-active:nth-child(3){
